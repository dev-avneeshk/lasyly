...
export function ServerArena({ server, labelFor }: { server: ReturnType<typeof useArenaServer>; labelFor: (seat: TeamId) => string }) {
  ...
  if (view.status === "lobby") return <WaitingRoom gameId={view.gameId} error={error} />
  ...
  return <div className="relative mx-auto max-w-[1280px] px-4 py-5 pb-40 md:px-6 md:pb-8"><ArenaMasthead player={lot?.player} />{error && <div className="mt-3 rounded-lg bg-[var(--color-danger)]/10 px-3 py-2 text-center text-sm text-[var(--color-danger)]">{error}</div>}<div className="mt-5 grid items-start gap-4 lg:grid-cols-[minmax(0,0.83fr)_minmax(24rem,1.32fr)_minmax(0,0.83fr)] lg:gap-5"><div className="order-2 lg:order-1 lg:pt-1"><BudgetPanel team={viewer} label={labelFor(viewer)} budget={view.budgets[viewer]} roster={view.rosters[viewer]} currentBid={lot?.currentBid} isHighBidder={lot?.highBidder === viewer} /></div><main className="order-1 flex min-w-0 flex-col items-center gap-3 lg:order-2">{lot && <section className="flex w-full items-center gap-3 rounded-[1.1rem] border border-white/[0.08] bg-[#11141e]/90 px-3.5 py-3 shadow-[0_14px_32px_rgba(0,0,0,0.16)]"><div className="relative h-12 w-12 shrink-0"><svg viewBox="0 0 56 56" className="h-12 w-12 -rotate-90"><circle cx="28" cy="28" r={radius} fill="none" stroke="rgba(255,255,255,0.09)" strokeWidth="4" /><motion.circle cx="28" cy="28" r={radius} fill="none" stroke={urgent ? "var(--color-danger)" : "#d4ff00"} strokeWidth="4" strokeLinecap="round" strokeDasharray={circumference} animate={{ strokeDashoffset: circumference * (1 - pct) }} transition={{ ease: "linear", duration: 0.1 }} /></svg><span className={cn("absolute inset-0 grid place-items-center text-base font-black tabular-nums", urgent ? "text-[var(--color-danger)]" : "text-[#f4f6fb]")}>{timeSec}</span></div><div className="min-w-0 flex-1"><div className="flex items-center justify-between gap-3 text-[8px] font-semibold uppercase tracking-[0.15em] text-[#8f9ab0]"><span>Lot {view.results.length + 1}</span><span>Current bid</span></div><div className="mt-1 flex items-end justify-between gap-3"><strong className="truncate text-xs font-bold" style={{ color: tone }}>{status}</strong><span className="text-xl font-black leading-none tabular-nums text-[#d4ff00]">${lot.currentBid}</span></div></div></section>}<div className="relative w-full"><div className={cn("transition-[filter] duration-200", justSold && "blur-[2px]")}><AnimatePresence mode="wait">{lot && <PlayerCard key={lot.player.id} player={lot.player} />}</AnimatePresence></div><AnimatePresence>{justSold && <SoldStamp name={justSold.name} winnerLabel={labelFor(justSold.winner)} price={justSold.price} />}</AnimatePresence></div>{lot && <div className="w-full"><BidControls state={state} humanSeat={viewer} minRaise={view.minRaise[viewer]} onBid={server.bid} onMax={() => { const max = view.budgets[viewer].maxAffordable; if (max > (lot?.currentBid ?? 0)) server.bid(max) }} onPass={() => server.pass()} /></div>}</main><div className="order-3 lg:pt-1"><BudgetPanel team={opponent} label={labelFor(opponent)} budget={view.budgets[opponent]} roster={view.rosters[opponent]} currentBid={lot?.currentBid} isHighBidder={lot?.highBidder === opponent} isAI={view.isAI[opponent]} /></div></div><AnimatePresence>{lastResult && <motion.p key={lastResult.playerId} initial={{ opacity: 0, y: 10 }} animate={{ opacity: 1, y: 0 }} className="mt-4 text-center text-sm text-[var(--color-text-muted)]">Last sold: <span className="font-semibold text-[var(--color-text-primary)]">{lastResult.playerName}</span> to {labelFor(lastResult.winner)} for ${lastResult.price}</motion.p>}</AnimatePresence></div>
}
