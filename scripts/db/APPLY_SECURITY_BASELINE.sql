...
-- ---------------------------------------------------------------------
-- 6. room_matches (CONDITIONAL — table may not exist in production)
-- ---------------------------------------------------------------------
DO $$
BEGIN
  IF EXISTS (
    ...
    EXECUTE 'ALTER TABLE public.room_matches ENABLE ROW LEVEL SECURITY';
    EXECUTE 'DROP POLICY IF EXISTS "room_matches_select_visible" ON public.room_matches';
    EXECUTE $p$CREATE POLICY "room_matches_select_visible"
      ON public.room_matches FOR SELECT
      USING (
        public.room_is_public(room_id)
        OR public.is_room_member(room_id, auth.uid())
      )$p$;
    EXECUTE 'DROP POLICY IF EXISTS "room_matches_insert_admin" ON public.room_matches';
    EXECUTE $p$CREATE POLICY "room_matches_insert_admin"
      ON public.room_matches FOR INSERT
      WITH CHECK (
        auth.uid() = added_by
        AND public.is_room_admin(room_id, auth.uid())
      )$p$;
    EXECUTE 'DROP POLICY IF EXISTS "room_matches_delete_admin" ON public.room_matches';
    EXECUTE $p$CREATE POLICY "room_matches_delete_admin"
      ON public.room_matches FOR DELETE
      USING (public.is_room_admin(room_id, auth.uid()))$p$;
    RAISE NOTICE 'room_matches RLS applied';
  ELSE
    RAISE NOTICE 'room_matches table does not exist — skipping section 6';
  END IF;
END $$;
