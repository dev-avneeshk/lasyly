/**
 * Top-level NFL Auction facade — ties the engine pieces together for the server
 * routes and (future) client runner.
 */

import type { AIDifficulty, NflGameResult, RosterState, Season, TeamId } from "./types"
import { simulateGame } from "./simulation"
import { analyzeResult } from "./analysis"
import { buildTeamProfile, type NflTeamProfile } from "./teamRating"

/**
 * A small success-probability edge applied to a CPU seat based on difficulty —
 * the standard "AI plays better on higher difficulty" knob, layered on top of
 * drafting. Human seats get 0.
 */
export function difficultyEdge(difficulty: AIDifficulty): number {
  switch (difficulty) {
    case "easy": return -0.03
    case "hard": return 0.035
    case "medium":
    default: return 0
  }
}

/** Run the drive simulation and attach the human-readable analysis. */
